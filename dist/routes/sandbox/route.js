...
exports.POST = (0, middleware_1.createRateLimitedApiRoute)(async ({ request }) => {
    const { fragment, userID, teamID, accessToken, } = await (0, validation_1.validateJsonBody)(request, validation_1.commonSchemas.sandboxRequest);
    if (!process.env.E2B_API_KEY) {
        console.error('E2B_API_KEY environment variable not found');
        throw (0, errors_1.createConfigError)('Code execution service is not configured. Please check environment settings.');
    }
    let sbx;
    try {
        // Create sandbox
        ...
        throw new errors_1.ApiException('An unexpected error occurred while setting up the sandbox.', 'unknown_error', 500, error?.message || 'Unknown error');
    }
}, {
