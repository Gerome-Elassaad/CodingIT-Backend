...
router.get('/github', async (req, res) => {
    try {
        const searchParams = (0, validation_1.validateSearchParams)(new URL(`${req.protocol}://${req.get('host')}${req.originalUrl}`), githubCallbackSchema.partial());
        if (searchParams.error) {
            console.error('GitHub OAuth error:', searchParams.error);
            return res.redirect(`/settings/integrations?error=${encodeURIComponent('GitHub authentication failed')}`);
        }
        if (!searchParams.code || !searchParams.state) {
            return res.redirect(`/settings/integrations?error=${encodeURIComponent('Missing authorization code')}`);
        }
        const supabase = (0, supabase_server_1.createServerClient)();
        const { data: { user }, error: authError } = await supabase.auth.getUser();
        if (authError || !user) {
            console.error('User authentication failed:', authError);
            return res.redirect(`/settings/integrations?error=${encodeURIComponent('User not authenticated')}`);
        }
